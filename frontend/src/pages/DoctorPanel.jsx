...
export default function DoctorPanel() {
  ...
  useEffect(() => {
    ...
    const onNewQuestion = (question) => setQuestions((prev) => [...prev, question]);
    ...
    const onGroupUpdated = () => fetchPendingQuestions();
  ...
  const handleAnswer = async (e) => {
    ...
    try {
      await api.post('/game/answer', { questionId: selectedQuestion.id, text: answerText });
    ...
    <div className="max-w-7xl mx-auto animate-fade-in-up">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 md:mb-8 mx-2 gap-4">
        <div className="pl-2 border-l-4 border-secondary">
          <h2 className="text-2xl md:text-4xl font-black text-white tracking-tight">{t('doctor.title')}</h2>
          <p className="text-sm md:text-gray-400 font-light mt-1">{t('doctor.subtitle')}</p>
        </div>
          ...
          <div className="flex-grow overflow-y-auto space-y-4 pr-2 custom-scrollbar relative z-10">
            {questions.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-4 opacity-50">
                <Clock size={48} />
                <p className="text-center font-medium">{t('doctor.no_queries')}</p>
              </div>
            ) : (
              questions.map((q) => (
                <button
                ...
                >
                  ...
                  <div className="flex flex-col gap-2">
                    <div className={`text-[10px] flex items-center gap-2 ${selectedQuestion?.id === q.id ? 'text-secondary font-bold' : 'text-gray-400'}`}>
                      ...
                      {t('doctor.patient')}: {q.author?.username || 'Unknown'}
                    </div>
                    ...
                    {q.status === 'accepted' ? (
                      <div className="flex items-center gap-2 px-2 py-1 rounded bg-secondary/10 border border-secondary/20 text-[10px] text-secondary">
                        <CheckCircle size={10} />
                        <span>{t('doctor.accepted_by')} {q.acceptor?.username === user.username ? t('scoreboard.you') : q.acceptor?.username}</span>
                      </div>
                    ) : (
                      <button
                      ...
                      >
                        {isAccepting ? t('doctor.processing') : (doctorHasAccepted ? t('doctor.busy') : t('doctor.accept'))}
                      </button>
                    )}
                  </div>
                </button>
        ...
        <div className="col-span-1 lg:col-span-2 h-full">
          {!selectedQuestion ? (
            <div className="h-full glass rounded-3xl flex flex-col items-center justify-center text-gray-500 p-10 text-center border border-glassBorder/30">
              ...
              <h3 className="text-xl font-bold text-white mb-2">{t('doctor.no_selected_title')}</h3>
              <p className="max-w-xs">{t('doctor.no_selected_desc')}</p>
            </div>
            ...
            <div className="h-full glass rounded-3xl flex flex-col items-center justify-center text-gray-500 p-10 text-center border border-glassBorder/30">
              ...
              <h3 className="text-xl font-bold text-white mb-2">{t('doctor.action_required')}</h3>
              <p className="max-w-xs">{t('doctor.action_desc')}</p>
              {selectedQuestion.status === 'accepted' && (
                <p className="mt-4 text-secondary/80 text-sm italic font-medium">
                  {t('doctor.being_handled')} {selectedQuestion.acceptor?.username || 'another specialist'}.
                </p>
              )}
            </div>
          ) : (
              ...
              <form onSubmit={handleAnswer} className="flex flex-col flex-grow relative z-10">
                <div className="flex-grow relative group">
                  ...
                  <textarea
                    ...
                    placeholder={t('doctor.answer_placeholder')}
                    value={answerText}
                ...
                <div className="flex justify-between items-center mt-6">
                  <p className="text-sm text-gray-500 italic hidden sm:block">{t('doctor.transmit_hint')}</p>
                  <button
                    type="submit"
                  ...
                  >
                    {isSubmitting ? t('doctor.transmitting') : t('doctor.transmit')} <Send size={20} className={isSubmitting ? "animate-pulse" : ""} />
                  </button>
                </div>
              </form>
            </div>
